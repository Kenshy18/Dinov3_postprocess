...
DEFAULT_ONNX = ROOT / "output/onnx/codino_decoder_b2_736x1280_msda_trt_plugin.onnx"
DEFAULT_ENGINE = ROOT / "checkpoints/codino/trt/codino_decoder_b2_736x1280_msda_plugin_fp16.engine"
...
def export_onnx(wrapper: torch.nn.Module, args: argparse.Namespace, feature_shapes: tuple[tuple[int, int], ...]) -> None:
    ...
    dummies = (
        torch.randn((args.num_query, args.batch_size, 256), device=args.device, dtype=export_dtype),
        torch.randn((total_tokens, args.batch_size, 256), device=args.device, dtype=export_dtype),
        torch.rand((args.batch_size, args.num_query, 4), device=args.device, dtype=export_dtype),
    )
    with torch.inference_mode():
