...
DEFAULT_OUTPUT = ROOT / "output/onnx/codino_dinov3_vitl_backbone_736x1280_fp32.onnx"
...
class CoDINOBackboneExportWrapper(torch.nn.Module):
    def __init__(self, backbone: torch.nn.Module, neck: torch.nn.Module | None = None) -> None:
        super().__init__()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--height", type=int, default=736, help="Padded model input height.")
    parser.add_argument("--width", type=int, default=1280, help="Padded model input width.")
    parser.add_argument("--batch-size", type=int, default=1)
    parser.add_argument("--fixed-batch", action="store_true")
    parser.add_argument("--with-neck", action="store_true", help="Export backbone + SFP neck multi-output features.")
...
def main() -> int:
    ...
    wrapper = CoDINOBackboneExportWrapper(model.backbone, model.neck if args.with_neck else None).to(args.device).eval()
    dummy = torch.randn(args.batch_size, 3, args.height, args.width, device=args.device, dtype=torch.float32)
    ...
    with torch.inference_mode():
        out = wrapper(dummy)
    output_shapes = [list(t.shape) for t in out] if isinstance(out, (tuple, list)) else [list(out.shape)]
    meta = {
