...
from trt_utils import default_tensorrt_site_packages, prepare_tensorrt
...
DEFAULT_ONNX = ROOT / "output/onnx/codino_mask_head_n1_736x1280.onnx"
DEFAULT_ENGINE = ROOT / "checkpoints/codino/trt/codino_mask_head_n1_736x1280_fp16.engine"
DEFAULT_CORE_ONNX = ROOT / "output/onnx/codino_mask_head_core_n1_736x1280.onnx"
DEFAULT_CORE_ENGINE = ROOT / "checkpoints/codino/trt/codino_mask_head_core_n1_736x1280_fp16.engine"
DEFAULT_TRT_SITE_PACKAGES = default_tensorrt_site_packages()


def _prepare_imports() -> None:
    for path in (ROOT, DINOV3_ROOT, CODINO_ROOT):
        path_str = str(path)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--trt-extra-site-packages", type=Path, default=DEFAULT_TRT_SITE_PACKAGES)
...
class MaskHeadExportWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        stage_instance_preds, _ = self.mask_head(instance_feats, semantic_feat, rois, roi_labels)
...
class MaskHeadCoreExportWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        for idx, stage in enumerate(self.mask_head.stages):
            instance_logits = self.mask_head.stage_instance_logits[idx](instance_feats)[:, :1]
            upsample_flag = self.mask_head.pre_upsample_last_stage or idx < len(self.mask_head.stages) - 1
            ...
            instance_feats = stage.upsample(fused_feats) if upsample_flag else fused_feats
...
def export_onnx(wrapper: torch.nn.Module, args: argparse.Namespace) -> None:
    ...
    instance_feats = torch.randn((args.num_rois, 256, 14, 14), device=args.device, dtype=export_dtype)
    if args.mode == "core":
        dummies = (
            instance_feats,
            torch.randn((args.num_rois, 256, 14, 14), device=args.device, dtype=export_dtype),
            torch.randn((args.num_rois, 128, 28, 28), device=args.device, dtype=export_dtype),
            torch.randn((args.num_rois, 64, 56, 56), device=args.device, dtype=export_dtype),
        )
        ...
        if args.num_rois > 0:
            rois[:, 0] = torch.arange(args.num_rois, device=args.device) % args.batch_size
            rois[:, 1] = 64.0
            rois[:, 2] = 64.0
            rois[:, 3] = 256.0
            rois[:, 4] = 256.0
    ...
    print(f"[sample] outputs={[tuple(t.shape) for t in samples]} dtypes={[str(t.dtype) for t in samples]}")
...
def build_engine(args: argparse.Namespace) -> None:
    ...
    config.set_memory_pool_limit(trt.MemoryPoolType.WORKSPACE, int(args.workspace_gb) << 30)
    ...
    print(f"[build] wrote {args.engine} size={args.engine.stat().st_size} elapsed={time.perf_counter() - t0:.2f}s")
...
def main() -> int:
    ...
    if not args.skip_export:
        ...
        wrapper_cls = MaskHeadCoreExportWrapper if args.mode == "core" else MaskHeadExportWrapper
