...
DEFAULT_ONNX = ROOT / "output/onnx/codino_query_encoder_b2_736x1280_msda_trt_plugin_sbc.onnx"
DEFAULT_ENGINE = ROOT / "checkpoints/codino/trt/codino_query_encoder_b2_736x1280_msda_plugin_sbc_fp16.engine"
...
class EncoderExportWrapper(torch.nn.Module):
    ...
    def forward(self, *features: torch.Tensor) -> torch.Tensor:
        if self.input_mode == "query":
            ...
            feat_flatten = torch.cat([f.flatten(2).transpose(1, 2) for f in features], 1)
...
def build_wrapper(model: torch.nn.Module, args: argparse.Namespace, feature_shapes: tuple[tuple[int, int], ...]):
    query_head = model.query_head
    transformer = query_head.transformer
    batch_size = int(args.batch_size)
    with torch.inference_mode():
        img_masks = torch.ones((batch_size, args.input_height, args.input_width), device=args.device)
        img_masks[:, : args.img_height, : args.img_width] = 0
        mlvl_masks = []
        mlvl_pos = []
        for height, width in feature_shapes:
            mask = torch.nn.functional.interpolate(img_masks[None], size=(height, width)).to(torch.bool).squeeze(0)
        ...
        spatial_shapes = torch.as_tensor(feature_shapes, dtype=torch.long, device=args.device)
        level_start_index = torch.cat((spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
        valid_ratios = torch.stack([transformer.get_valid_ratio(m) for m in mlvl_masks], 1)
        reference_points = transformer.get_reference_points(spatial_shapes, valid_ratios, device=args.device)
        lvl_pos = []
        for lvl, pos in enumerate(mlvl_pos):
            lvl_pos.append(pos.flatten(2).transpose(1, 2) + transformer.level_embeds[lvl].view(1, 1, -1))
        pos_flatten = torch.cat(lvl_pos, 1).permute(1, 0, 2).contiguous()
    return EncoderExportWrapper(
        transformer,
        mask_flatten,
        pos_flatten,
        spatial_shapes,
...
def export_onnx(wrapper: torch.nn.Module, args: argparse.Namespace, feature_shapes: tuple[tuple[int, int], ...]) -> None:
    ...
    if args.input_mode == "query":
        ...
        dummies = (torch.randn((total_tokens, args.batch_size, 256), device=args.device, dtype=export_dtype),)
        input_names = ["query"]
    else:
        dummies = tuple(
            torch.randn((args.batch_size, 256, height, width), device=args.device, dtype=export_dtype)
            for height, width in feature_shapes
        )
