"""Common JSONL contract for detector outputs consumed by postprocess.
...
def _to_int(value: Any, field: str) -> int:
    try:
        ...
        raise DetectionJsonlContractError(f"{field} must be an integer: {value!r}") from exc
...
def _to_float(value: Any, field: str) -> float:
    try:
        ...
        raise DetectionJsonlContractError(f"{field} must be numeric: {value!r}") from exc
...
def _has_mask(src: dict[str, Any]) -> bool:
    ...
    return isinstance(polygons, list) and bool(polygons) or isinstance(segmentation, list) and bool(segmentation)


def normalize_detection(src: Any) -> dict[str, Any]:
    if not isinstance(src, dict):
        raise DetectionJsonlContractError(f"detection must be an object: {type(src).__name__}")
    ...
    bbox_xyxy = _normalize_bbox_xyxy(src)
    if bbox_xyxy is not None:
        out["bbox_xyxy"] = bbox_xyxy
        out["bbox"] = [bbox_xyxy[0], bbox_xyxy[1], bbox_xyxy[2] - bbox_xyxy[0], bbox_xyxy[3] - bbox_xyxy[1]]
    ...
    polygons = src.get("polygons", src.get("segmentation"))
    if polygons is not None:
        if not isinstance(polygons, list):
            raise DetectionJsonlContractError("polygons/segmentation must be a list")
...
def normalize_frame_record(src: Any) -> dict[str, Any]:
    if not isinstance(src, dict):
        raise DetectionJsonlContractError(f"frame record must be an object: {type(src).__name__}")
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, 1):
            ...
            try:
                ...
                raise DetectionJsonlContractError(f"{path}:{line_no}: invalid JSON: {exc}") from exc
            ...
            stats.first_frame_index = frame_index if stats.first_frame_index is None else min(stats.first_frame_index, frame_index)
            stats.last_frame_index = frame_index if stats.last_frame_index is None else max(stats.last_frame_index, frame_index)
