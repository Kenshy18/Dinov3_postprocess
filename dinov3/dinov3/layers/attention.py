...
def _use_xformers() -> bool:
    return _HAS_XFORMERS and _env_flag("DINOV3_USE_XFORMERS")
...
class SelfAttention(nn.Module):
    ...
    def compute_attention(self, qkv: Tensor, attn_bias=None, rope=None) -> Tensor:
        ...
        if _use_xformers():
            ...
            x = xops.memory_efficient_attention(q, k, v, p=p, scale=self.scale)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, is_causal: bool = True) -> Tensor:
        ...
        if _use_xformers():
            ...
            x = xops.memory_efficient_attention(q, k, v, attn_bias=attn_bias, p=p, scale=self.scale)
            x = x.reshape(B, N, C)
