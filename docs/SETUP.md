...
The generated runtime profile is used by the integrated pipeline defaults. Setup measures DINOv3, EVA02, and Co-DINO candidate batch sizes sequentially with production-like runtime options, writes the result to `.runtime/runtime_benchmark.json`, and deletes temporary benchmark output afterward. By default it uses the first video under `input/` for 240 frames and falls back to a temporary dummy video when no input video exists; set `BATCH_BENCHMARK_INPUT=/path/to/sample.mp4` to force a representative real video. The selected GUI Python/venv, runtime profile path, benchmark result path, and TensorRT engine paths are written to `.runtime/gui_runtime.env`, which `apps/qt_ui/run_app.sh` sources before launching the application. `UI/run_app.sh` remains a compatibility wrapper. `.runtime/runtime_profile.json`, `.runtime/runtime_benchmark.json`, and `.runtime/gui_runtime.env` are local-machine state and are gitignored; keep the tracked guideline in `configs/runtime_profile.example.json` up to date instead. Legacy `configs/runtime_profile.json` is still read as a fallback during migration. If benchmarking cannot select a value, setup falls back to conservative defaults that keep EVA02 batch-size low on GPUs below 16 GiB VRAM to avoid CUDA unified/shared-memory fallback.
