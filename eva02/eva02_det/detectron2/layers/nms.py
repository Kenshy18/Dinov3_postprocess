...
def batched_nms(
    ...
    try:
        ...
        # Fallback for GPUs whose torchvision NMS kernels are not built (e.g., sm_120).
        if boxes.is_cuda and "no kernel image is available" in str(exc).lower():
            boxes_cpu = boxes.float().cpu()
            scores_cpu = scores.float().cpu()
            idxs_cpu = idxs.cpu()
            keep = box_ops.batched_nms(boxes_cpu, scores_cpu, idxs_cpu, iou_threshold)
