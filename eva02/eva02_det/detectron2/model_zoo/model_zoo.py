...
def get_config_file(config_path):
    ...
    if not os.path.exists(cfg_file):
        # Editable / in-repo layouts may keep builtin configs under eva02_det/configs
        # without packaging them as detectron2.model_zoo resources.
        repo_cfg_file = Path(__file__).resolve().parents[2] / "configs" / config_path
        if repo_cfg_file.exists():
            return str(repo_cfg_file)
