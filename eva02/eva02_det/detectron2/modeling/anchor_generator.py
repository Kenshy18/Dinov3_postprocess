...
class DefaultAnchorGenerator(nn.Module):
    ...
    def _grid_anchors(self, grid_sizes: List[List[int]]):
        ...
        """
        use_cache = os.environ.get("EVA_RPN_ANCHOR_CACHE", "0") == "1"
        buffers: List[torch.Tensor] = [x[1] for x in self.cell_anchors.named_buffers()]
        cache_key = None
        if use_cache and buffers:
            device = buffers[0].device
            cache_key = (
                tuple((int(size[0]), int(size[1])) for size in grid_sizes),
                tuple(int(x) for x in self.strides),
                device.type,
                -1 if device.index is None else int(device.index),
            )
            cached = self._grid_anchors_cache.get(cache_key)
        ...
        if cache_key is not None:
            self._grid_anchors_cache[cache_key] = anchors
        return anchors
