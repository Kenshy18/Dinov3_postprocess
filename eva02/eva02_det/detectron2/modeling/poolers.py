...
def convert_boxes_to_pooler_format(box_lists: List[Boxes], fixed_boxes_per_image: int = 0):
    """
    ...
    if fixed_boxes_per_image > 0:
        sizes = torch.full(
            (len(box_lists),),
            int(fixed_boxes_per_image),
            dtype=torch.int64,
            device=boxes.device,
        )
    else:
...
class ROIPooler(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes]):
        ...
        fixed_boxes_per_image = int(getattr(self, "fixed_boxes_per_image", 0))
        pooler_fmt_boxes = convert_boxes_to_pooler_format(
            box_lists,
            fixed_boxes_per_image=fixed_boxes_per_image,
        )
