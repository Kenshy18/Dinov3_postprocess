...
from detectron2.layers import batched_nms, cat, move_device_like, nms
...
def _rpn_nms_keep(
    ...
    impl = os.environ.get("EVA_RPN_NMS_IMPL", "batched").strip().lower()
    if impl == "per_level_slices" and level_slices is not None:
        if boxes.numel() == 0:
            return torch.empty((0,), dtype=torch.int64, device=boxes.device)

        keep_chunks = []
        for start, end in level_slices:
            start_i = int(start)
            end_i = int(end)
            if end_i <= start_i:
                continue
            level_keep = nms(boxes[start_i:end_i].float(), scores[start_i:end_i], iou_threshold)
            keep_chunks.append(level_keep + start_i)
    ...
    if impl != "per_level":
        return batched_nms(boxes, scores, level_ids, iou_threshold)

    if boxes.numel() == 0:
        return torch.empty((0,), dtype=torch.int64, device=boxes.device)

    keep_chunks = []
    unique_levels = torch.unique(level_ids)
    for level in unique_levels:
        mask = level_ids == level
        if not torch.any(mask):
            continue
        level_indices = torch.nonzero(mask, as_tuple=False).squeeze(1)
        level_keep = nms(boxes[level_indices].float(), scores[level_indices], iou_threshold)
        keep_chunks.append(level_indices[level_keep])
...
def find_top_rpn_proposals(
    ...
    for level_id, (proposals_i, logits_i) in enumerate(zip(proposals, pred_objectness_logits)):
        ...
        level_sizes.append(num_proposals_i)
        level_ids.append(
            move_device_like(
    ...
    for level_size in level_sizes:
        size_i = int(level_size)
        level_slices.append((offset, offset + size_i))
        offset += size_i
    ...
    for n, image_size in enumerate(image_sizes):
        ...
        keep = _rpn_nms_keep(boxes.tensor, scores_per_img, lvl, nms_thresh, level_slices=level_slices)
