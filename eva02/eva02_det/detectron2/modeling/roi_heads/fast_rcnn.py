...
# ===== MMCV Soft-NMS (optional) & PyTorch NMS (fallback) =====
try:
    from mmcv.ops import soft_nms as mmcv_soft_nms  # fast path (if mmcv compiled for current torch)
except Exception:
    mmcv_soft_nms = None
...
_FAST_RCNN_ONECLASS_FASTPATH = os.environ.get("EVA_FAST_RCNN_ONECLASS_FASTPATH", "0") == "1"
...
def fast_rcnn_inference(
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            ...
            use_soft_nms=use_soft_nms, method=method, iou_threshold=iou_threshold, sigma=sigma,
            class_wise=class_wise, override_score_thresh=override_score_thresh,
            keep_all=keep_all_before_merge,
        )
...
def fast_rcnn_inference_single_image(
    ...
    class_wise: bool = False,
    override_score_thresh: float = None,
    keep_all: bool = False,  # for avoiding repeated nms when tta with single input
):
    """
    ...
    if override_score_thresh is not None:
        score_thresh = override_score_thresh

    # 1. Filter results based on detection scores. It can make NMS more efficient
    #    by filtering out low-confidence detections.
    use_oneclass_fastpath = (
        _FAST_RCNN_ONECLASS_FASTPATH
        and scores.shape[1] == 1
        and num_bbox_reg_classes == 1
    )
    if use_oneclass_fastpath:
        flat_scores = scores[:, 0]
        keep_mask = flat_scores > score_thresh
        filter_rows = keep_mask.nonzero(as_tuple=False).squeeze(1)
        boxes = boxes[filter_rows, 0]
        scores = flat_scores[filter_rows]
        filter_classes = torch.zeros_like(filter_rows)
    else:
        filter_mask = scores > score_thresh  # R x K
        # R' x 2. First column contains indices of the R predictions;
        # Second column contains indices of classes.
        filter_inds = filter_mask.nonzero()
        filter_rows = filter_inds[:, 0]
        filter_classes = filter_inds[:, 1]
    ...
    if keep_all:
        ...
        result.pred_classes = filter_classes
        return result, filter_rows

    # ===================== Soft-NMS path (with fallback) =====================
    if use_soft_nms:
        if mmcv_soft_nms is not None:
            # -------- Original MMCV Soft-NMS path --------
            if not class_wise:
                ...
                dets, keep = mmcv_soft_nms(
                    boxes=boxes_for_nms, scores=scores, iou_threshold=iou_threshold,
                    sigma=sigma, min_score=1e-3, method=method
                )
                boxes = boxes[keep]
                scores = dets[:, -1]  # scores updated in soft-nms

            result = Instances(image_shape)
            result.pred_boxes = Boxes(boxes)
            result.scores = scores
            filter_rows = filter_rows[keep]
            filter_classes = filter_classes[keep]
            result.pred_classes = filter_classes
            return result, filter_rows

        # -------- Fallback to PyTorch/detectron2 hard-NMS --------
        warnings.warn(
            "mmcv.ops.soft_nms not available; falling back to hard NMS (torchvision/detectron2)."
        )
        # if torchvision is present, use it; else use detectron2.batched_nms.
        iou_thr = float(iou_threshold) if iou_threshold is not None else float(nms_thresh)

        if not class_wise:
            if torch_nms is not None:
                keep = torch_nms(boxes, scores, iou_thr)
            else:
                # One-class dummy labels for batched_nms
                dummy_labels = torch.zeros_like(scores, dtype=torch.long)
                keep = batched_nms(boxes, scores, dummy_labels, iou_thr)
        else:
            # per-class NMS
            keep_list = []
            ...
            for c in unique_classes:
                ...
                if mask.any():
                    if torch_nms is not None:
                        kept_local = torch_nms(boxes[mask], scores[mask], iou_thr)
                    else:
                        kept_local = batched_nms(
                            boxes[mask], scores[mask],
                            torch.zeros_like(scores[mask], dtype=torch.long), iou_thr
                        )
                    # map local indices back to global indices
                    global_idx = torch.where(mask)[0][kept_local]
                    keep_list.append(global_idx)
            keep = torch.cat(keep_list, dim=0) if len(keep_list) > 0 else torch.empty(0, dtype=torch.long, device=boxes.device)

        # Keep order by descending score (torchvision.nms doesn't guarantee order)
        if keep.numel() > 0:
            order = scores[keep].argsort(descending=True)
            keep = keep[order]
        # (Note) original soft-nms branch did not enforce top-k; we keep parity here.
        ...
        result.pred_classes = filter_classes
        return result, filter_rows
    # =================== End Soft-NMS path (with fallback) ===================

    # 2. Apply standard NMS.
