...
class ImageList(object):
    ...
    def from_tensors(
        ...
        use_channels_last = (
            len(tensors[0].shape) == 3
            and tensors[0].device.type == "cuda"
            and os.environ.get("DETECTRON2_CHANNELS_LAST", "0").lower() in ("1", "true", "yes", "on")
        )
        ...
        if len(tensors) == 1:
            ...
            if use_channels_last:
                batched_imgs = batched_imgs.contiguous(memory_format=torch.channels_last)
        else:
            ...
            if use_channels_last and not torch.jit.is_tracing() and not torch.jit.is_scripting():
                fill_device = tensors[0].device if device is None else device
                batched_imgs = torch.empty(
                    batch_shape,
