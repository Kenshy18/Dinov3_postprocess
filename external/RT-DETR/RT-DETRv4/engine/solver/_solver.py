...
from ..misc.wandb_utils import WandbLogger
from ..misc.memory_debug import MemoryDebugTracker
...
class BaseSolver(object):
    ...
    def _setup(self):
        ...
        raw_output_dir = Path(cfg.output_dir)
        typed_roots = {
            'pth',
            'json',
            'csv',
        ...
        if raw_output_dir.parent.name in typed_roots:
            self.output_dir = raw_output_dir
        else:
            base_root = raw_output_dir.parent if raw_output_dir.parent != Path('.') else Path('outputs')
            self.output_dir = base_root / 'pth' / raw_output_dir.name
        cfg.output_dir = str(self.output_dir)
        self.output_dir.mkdir(parents=True, exist_ok=True)
        self.run_name = self.output_dir.name
        self.project_output_root = self.output_dir.parent.parent
        self.meta_dir = self.project_output_root / 'meta' / self.run_name
        self.meta_dir.mkdir(parents=True, exist_ok=True)
        self.writer = cfg.writer
        self.wandb_logger = WandbLogger(cfg)
        if getattr(self.wandb_logger, 'enabled', False):
            atexit.register(self.wandb_logger.finish)

        self.memory_debugger = MemoryDebugTracker(cfg, output_dir=self.meta_dir, wandb_logger=self.wandb_logger)
