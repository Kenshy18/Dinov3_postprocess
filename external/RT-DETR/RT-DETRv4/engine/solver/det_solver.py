...
class DetSolver(BaseSolver):
    ...
    def fit(self, ):
        ...
        for epoch in range(start_epoch, args.epoches):
            ...
            train_stats, grad_percentages = train_one_epoch(
                ...
                writer=self.writer,
                wandb_logger=self.wandb_logger,
                memory_debugger=self.memory_debugger,
            ...
            if self.wandb_logger is not None and getattr(self.wandb_logger, 'enabled', False):
                self.wandb_logger.log_eval_epoch(test_stats, epoch)
            ...
            if self.output_dir and dist_utils.is_main_process():
                with (self.meta_dir / "log.txt").open("a") as f:
                    f.write(json.dumps(log_stats) + "\n")

                # for evaluation logs
                if coco_evaluator is not None:
                    (self.meta_dir / 'eval').mkdir(exist_ok=True)
    ...
    def val(self, ):
        ...
        test_stats, coco_evaluator = evaluate(module, self.criterion, self.postprocessor,
                self.val_dataloader, self.evaluator, self.device, wandb_logger=self.wandb_logger, epoch=self.last_epoch)
