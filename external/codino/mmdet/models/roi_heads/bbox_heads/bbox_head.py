    ...
    def loss(self,
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                # DDP safety:
                # Some ranks can end up with 0 RoIs (e.g., empty proposals after
                # numerical issues). In that case cls_score has 0 elements and
                # the original implementation would skip adding `loss_cls`/`acc`,
                # which then crashes DDP in `_parse_losses` due to mismatched keys.
                zero = cls_score.sum() * 0.0
                losses['loss_cls'] = zero
                if not self.custom_activation:
                    losses['acc'] = zero
