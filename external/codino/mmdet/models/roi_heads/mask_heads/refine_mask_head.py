...
class SFMStage(nn.Module):
    ...
    def forward(self, instance_feats, semantic_feat, semantic_pred, rois, roi_labels):
        ...
        # RoIAlign requires rois and feats to have the same dtype (MMCV ext).
        # Under AMP, semantic_feat/semantic_pred can become fp16 while rois stays fp32.
        rois_fp32 = rois.float() if rois.dtype != torch.float32 else rois
        semantic_feat_fp32 = semantic_feat.float() if semantic_feat.dtype != rois_fp32.dtype else semantic_feat
        ins_semantic_feats = self.semantic_roi_extractor([semantic_feat_fp32,], rois_fp32)
...
class RefineMaskHead(nn.Module):
    ...
    def get_seg_masks(self, mask_pred, det_bboxes, det_labels, rcnn_test_cfg, ori_shape, scale_factor, rescale):
        ...
        # Avoid per-instance .cpu() calls (each call can trigger a sync/copy).
        # Copy once, then create views for the list output expected by mmdet.
        im_mask_cpu = im_mask.cpu().numpy()
        im_segms = [im_mask_cpu[i] for i in range(N)]
        return im_segms
