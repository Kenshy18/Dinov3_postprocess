...
class CoDeformDETRHead(DETRHead):
    ...
    def _get_target_single(self,
        ...
        pos_gt_bboxes_normalized = (
            sampling_result.pos_gt_bboxes.to(bbox_pred.dtype) / factor)
        pos_gt_bboxes_targets = bbox_xyxy_to_cxcywh(
            pos_gt_bboxes_normalized).to(bbox_pred.dtype)
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            scores, bbox_index = scores.topk(k)
            bbox_pred = bbox_pred[bbox_index]
        ...
        if det_bboxes.numel() == 0:
            return det_bboxes.new_zeros((0, 5)), det_labels

        if with_nms:
            cfg = self.test_cfg
            # soft_nms does not support half; ensure float32 inputs
