...
class CoDETR(BaseDetector):
    ...
    def simple_test_query_head(self, img, img_metas, proposals=None, rescale=False):
        ...
        if hasattr(self, "mask_head"):
            # NOTE: results_list contains per-image tensors whose first
            # dimension (#detections) can vary across images. Do not torch.stack.
            det_bboxes = [det_bbox for det_bbox, _ in results_list]
            det_labels = [det_label for _, det_label in results_list]
    ...
    def simple_test_mask(self,
        ...
        segm_results = []
        mask_scores = []
        # Support both batched tensors and per-image lists (batched inference).
        if isinstance(det_bboxes, (list, tuple)):
            det_bboxes_list = list(det_bboxes)
        else:
            det_bboxes_list = [det_bboxes[i] for i in range(len(img_metas))]

        if isinstance(det_labels, (list, tuple)):
            det_labels_list = list(det_labels)
        else:
            det_labels_list = [det_labels[i] for i in range(len(img_metas))]

        for img_idx in range(len(img_metas)):
            ori_shape = img_metas[img_idx]['ori_shape']
            scale_factor = img_metas[img_idx]['scale_factor']
            det_bboxes_i = det_bboxes_list[img_idx]
            det_labels_i = det_labels_list[img_idx]
            x_i = x
            if isinstance(x, (list, tuple)) and len(x) > 0 and isinstance(x[0], torch.Tensor):
                batch_size = x[0].shape[0]
                if batch_size == len(img_metas):
                    x_i = [
                        feat[img_idx: img_idx + 1] if isinstance(feat, torch.Tensor) and feat.shape[0] == batch_size else feat
                        for feat in x
                    ]
            # In batched inference, per-image detections may contain a single
            # box/label and come as 1D tensors. Normalize shapes here.
            if isinstance(det_bboxes_i, torch.Tensor) and det_bboxes_i.dim() == 1:
                det_bboxes_i = det_bboxes_i.unsqueeze(0)
            if isinstance(det_labels_i, torch.Tensor) and det_labels_i.dim() == 0:
                det_labels_i = det_labels_i.unsqueeze(0)
            if det_bboxes_i.shape[0] == 0:
                ...
                if rescale and not isinstance(scale_factor, float):
                    scale_factor = torch.from_numpy(scale_factor).to(
                        det_bboxes_i.device, dtype=det_bboxes_i.dtype)
                _bboxes = det_bboxes_i[:, :4] * scale_factor if rescale else det_bboxes_i
                paste_bboxes = _bboxes
                if rescale:
                    # The local eval pipeline uses centered letterbox padding.
                    # Query boxes are rescaled by scale_factor only, so they
                    # still include pad offsets. Use padded boxes for ROI
                    # extraction, but remove the pad before mask pasting into
                    # the original image canvas.
                    img_shape = img_metas[img_idx].get('img_shape', img_metas[img_idx].get('pad_shape', ori_shape))
                ...
                # Keep ROI dtype consistent with feature dtype for mmcv ops (e.g., RoIAlign).
                feat_dtype = x_i[0].dtype if isinstance(x_i, (list, tuple)) and len(x_i) > 0 else mask_rois.dtype
                ...
                segm_result = [[] for _ in range(self.mask_head.stage_num_classes[0])]
                mask_score = [[] for _ in range(self.mask_head.stage_num_classes[0])]
                for i in range(0, det_labels_i.shape[0], interval):
                    mask_results = self._mask_forward(x_i, mask_rois[i: i + interval], det_labels_i[i: i + interval])
                    ...
                    if hasattr(self, "mask_iou_head"):
                        ...
                        chunk_mask_score = self.mask_iou_head.get_mask_scores(
                            mask_iou_pred,
                            det_bboxes_i[i: i + interval],
                            det_labels_i[i: i + interval],
                            return_score=True)
                    ...
                    chunk_segm_result = self.mask_head.get_seg_masks(
                        instance_pred, paste_bboxes[i: i + interval], det_labels_i[i: i + interval],
                        self.rcnn_test_cfg, ori_shape, scale_factor, rescale)

                    if hasattr(self, "mask_iou_head"):
                        for c, segm, score in zip(det_labels_i[i: i + interval], chunk_segm_result, chunk_mask_score):
                            segm_result[c].append(segm)     
                            mask_score[c].append(score)                   
                    else:
                        for c, segm in zip(det_labels_i[i: i + interval], chunk_segm_result):
                            segm_result[c].append(segm)
