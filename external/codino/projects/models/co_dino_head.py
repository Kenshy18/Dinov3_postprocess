...
class CoDINOHead(CoDeformDETRHead):
    ...
    @force_fp32(apply_to=('all_cls_scores', 'all_bbox_preds',
                          'enc_topk_scores', 'enc_topk_anchors'))
    def loss(self,
             all_cls_scores,
             all_bbox_preds,
             enc_topk_scores,
    ...
    @force_fp32(apply_to=('dn_cls_scores', 'dn_bbox_preds'))
    def loss_dn_single(self, dn_cls_scores, dn_bbox_preds, gt_bboxes_list,
        ...
        if len(cls_scores) > 0:
            ...
            scores = label_weights.new_zeros(labels.shape, dtype=cls_scores.dtype)
        ...
        bboxes = bbox_cxcywh_to_xyxy(bbox_preds.float()) * factors.float()
    ...
    def _get_dn_target_single(self, dn_bbox_pred, gt_bboxes, gt_labels,
        ...
        if len(gt_labels) > 0:
            # indices [0, 1, ..., len(gt_labels)-1] on same device as gt_labels
            t = torch.arange(len(gt_labels),
            ...
            pos_inds = (torch.arange(num_groups,
                                     device=gt_labels.device,
                                     dtype=torch.long)
                        * single_pad).unsqueeze(1) + t
            pos_inds = pos_inds.flatten()
        else:
            pos_inds = pos_assigned_gt_inds = torch.tensor(
                [], device=gt_labels.device, dtype=torch.long)
        ...
        factor = dn_bbox_pred.new_tensor([img_w, img_h, img_w,
                                          img_h]).unsqueeze(0)
        gt_bboxes_normalized = gt_bboxes.to(dn_bbox_pred.dtype) / factor
        gt_bboxes_targets = bbox_xyxy_to_cxcywh(
            gt_bboxes_normalized).to(dn_bbox_pred.dtype)
