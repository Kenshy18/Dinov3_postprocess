...
ENGINE_PATH="${ENGINE_PATH:-$ROOT_DIR/checkpoints/trt/dinov3_backbone_fp32_720x1280_dynamic_bf16_forced_b1_8_8.engine}"
...
BATCH_BENCHMARK_TIE_FPS_RATIO="${BATCH_BENCHMARK_TIE_FPS_RATIO:-0.03}"
BATCH_BENCHMARK_INPUT="${BATCH_BENCHMARK_INPUT:-auto}"
...
EVA02_BENCHMARK_COMPILE_BACKBONE="${EVA02_BENCHMARK_COMPILE_BACKBONE:-max-autotune}"
...
if [[ "$RUN_BATCH_BENCHMARK" == "1" ]]; then
  ...
  if [[ -n "$BATCH_BENCHMARK_INPUT" ]]; then
    echo "[SETUP] batch benchmark input video: $BATCH_BENCHMARK_INPUT"
  fi
  if ! BATCH_BENCHMARK_DETECTORS="$BATCH_BENCHMARK_DETECTORS" \
    BATCH_BENCHMARK_FRAMES="$BATCH_BENCHMARK_FRAMES" \
    BATCH_BENCHMARK_INPUT="$BATCH_BENCHMARK_INPUT" \
    ...
    BATCH_BENCHMARK_TIE_FPS_RATIO="$BATCH_BENCHMARK_TIE_FPS_RATIO" \
    ...
    EVA02_BENCHMARK_COMPILE_BACKBONE="$EVA02_BENCHMARK_COMPILE_BACKBONE" \
...
mkdir -p "$(dirname "$GUI_RUNTIME_ENV")"
{
  ...
  printf 'EVA02_BENCHMARK_COMPILE_BACKBONE=%q\n' "$EVA02_BENCHMARK_COMPILE_BACKBONE"
  printf 'BATCH_BENCHMARK_TIE_FPS_RATIO=%q\n' "$BATCH_BENCHMARK_TIE_FPS_RATIO"
  printf 'BATCH_BENCHMARK_INPUT=%q\n' "$BATCH_BENCHMARK_INPUT"
